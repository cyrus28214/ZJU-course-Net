...
+ 实现简单直观,`std::vector` 是C++中最基础、最易用的容器之一。add_route（添加路由）操作可以非常简单地通过 `_routing_table.push_back()`（在末尾追加）来实现。
+ 易于实现最长前缀匹配：路由转发的核心是“最长前缀匹配”算法。使用 `vector`，我可以在 `route_one_datagram` 方法中通过一个简单的 for 循环，线性遍历路由表中的所有条目。在循环中，我可以逐个检查数据包的目的IP是否匹配当前路由条目，并维护一个变量（如 `longest_prefix` 和 `best_match_idx`）来记录“到目前为止匹配上的最长前缀”。这种线性扫描的方式虽然在性能上不是最优的，但它逻辑最清晰，最容易正确实现算法要求。
+ 在本实验的测试中，路由表的规模非常小（条目数 $N$ 很小）。因此，每次查找都进行 $O(N)$ 复杂度的线性遍历，其性能完全可以接受，足以通过所有测试。实验的重点是让我们理解路由的逻辑，而不是优化大规模路由的查找性能，所以暂时不必使用Trie树来实现。
